...
from .models import User, Post, Friends
...
def getNoteById(id, owner=True):
    ...
    if owner:
        ...
        shared = Friends.query.filter_by(post_id=id, user_id=current_user.id).first()
...
def shareNoteById(note_id, email):
    ...
    *login required*
    ...
    if note:
        ...
        if getUser:
            memberExist = Friends.query.filter_by(post_id=note_id, user_id=getUser.id).first() is not None
...
def delMemberByNoteId(getPost_id, getUser_id):
    ...
    *login required*
    ...
    if owner:
        member = Friends.query.filter_by(post_id=getPost_id, user_id=getUser_id).first()
